...
case $1 in
  ...
  *)
    ...
    echo
    echo "Usage:  Require all four arguments ---> `basename $0` <create | delete> --profile=<SSO_PROFILE_NAME> --snowflake_account_identifier=<SNOWFLAKE_ACCOUNT_IDENTIFIER> --snowflake_admin_user=<SNOWFLAKE_ADMIN_USER> --snowflake_password=<SNOWFLAKE_PASSWORD> --snowflake_warehouse=<SNOWFLAKE_WAREHOUSE> --secrets_root_path=<SECRETS_ROOT_PATH> --new_admin_service_user=<NEW_ADMIN_SERVICE_USER>"
    echo
...
do
    case $arg in
        ...
        *"--snowflake_admin_user="*)
            arg_length=23
            snowflake_admin_user=${arg:$arg_length:$(expr ${#arg} - $arg_length)};;
        *"--snowflake_password="*)
            arg_length=21
        ...
        *"--new_admin_service_user="*)
            arg_length=25
            new_admin_service_user=${arg:$arg_length:$(expr ${#arg} - $arg_length)};;
    esac
done
...
# Check required --snowflake_admin_user argument was supplied
if [ -z $snowflake_admin_user ]
then
    echo
    echo "(Error Message 004)  You did not include the proper use of the --snowflake_admin_user=<SNOWFLAKE_ADMIN_USER> argument in the call."
    echo
    echo "Usage:  Require all four arguments ---> `basename $0 $1` --profile=<SSO_PROFILE_NAME> --snowflake_account_identifier=<SNOWFLAKE_ACCOUNT_IDENTIFIER> --snowflake_admin_user=<SNOWFLAKE_ADMIN_USER> --snowflake_password=<SNOWFLAKE_PASSWORD> --snowflake_warehouse=<SNOWFLAKE_WAREHOUSE> --secrets_root_path=<SECRETS_ROOT_PATH> --new_admin_service_user=<NEW_ADMIN_SERVICE_USER>"
    echo
    exit 85 # Common GNU/Linux Exit Code for 'Interrupted system call should be restarted'
...
then
    echo
    echo "(Error Message 006)  You did not include the proper use of the --snowflake_admin_user=<SNOWFLAKE_WAREHOUSE> argument in the call."
    echo
    echo "Usage:  Require all four arguments ---> `basename $0 $1` --profile=<SSO_PROFILE_NAME> --snowflake_account_identifier=<SNOWFLAKE_ACCOUNT_IDENTIFIER> --snowflake_admin_user=<SNOWFLAKE_ADMIN_USER> --snowflake_password=<SNOWFLAKE_PASSWORD> --snowflake_warehouse=<SNOWFLAKE_WAREHOUSE> --secrets_root_path=<SECRETS_ROOT_PATH> --new_admin_service_user=<NEW_ADMIN_SERVICE_USER>"
    echo
    exit 85 # Common GNU/Linux Exit Code for 'Interrupted system call should be restarted'
...
# Check required --new_admin_service_user argument was supplied
if [ -z $new_admin_service_user ]
then
    echo
    echo "(Error Message 008)  You did not include the proper use of the --new_admin_service_user=<NEW_ADMIN_SERVICE_USER> argument in the call."
    echo
    echo "Usage:  Require all four arguments ---> `basename $0 $1` --profile=<SSO_PROFILE_NAME> --snowflake_account_identifier=<SNOWFLAKE_ACCOUNT_IDENTIFIER> --snowflake_admin_user=<SNOWFLAKE_ADMIN_USER> --snowflake_password=<SNOWFLAKE_PASSWORD> --snowflake_warehouse=<SNOWFLAKE_WAREHOUSE> --secrets_root_path=<SECRETS_ROOT_PATH> --new_admin_service_user=<NEW_ADMIN_SERVICE_USER>"
    echo
    exit 85 # Common GNU/Linux Exit Code for 'Interrupted system call should be restarted'
...
# Function to handle the service user creation error
user_updater_handler() {
    snow sql -q "ALTER USER ${new_admin_service_user} SET RSA_PUBLIC_KEY=\"`cat public_key_1.pub`\";" --temporary-connection --role ACCOUNTADMIN
    snow sql -q "ALTER USER ${new_admin_service_user} UNSET RSA_PUBLIC_KEY_2;" --temporary-connection --role ACCOUNTADMIN
...
# Set the trap to catch service user creation error
...
then
    ...
    snow sql -q "CREATE USER ${new_admin_service_user} TYPE=SERVICE RSA_PUBLIC_KEY=\"`cat public_key_1.pub`\" DEFAULT_ROLE=PUBLIC;" --temporary-connection --role ACCOUNTADMIN
    snow sql -q "GRANT ROLE ACCOUNTADMIN TO USER ${new_admin_service_user};" --temporary-connection --role ACCOUNTADMIN
    snow sql -q "GRANT ROLE SECURITYADMIN TO USER ${new_admin_service_user};" --temporary-connection --role ACCOUNTADMIN
    ...
    snow sql -q "DROP USER IF EXISTS ${new_admin_service_user};" --temporary-connection --role ACCOUNTADMIN
